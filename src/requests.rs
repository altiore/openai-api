...
fn deal_response(response: Result<ureq::Response, ureq::Error>, sub_url: &str) -> ApiResult<Json> {
    match response {
        ...
        Err(err) => match err {
            ureq::Error::Status(status, response) => match response.into_json::<Json>() {
                Ok(error_msg) => Err(Error::ApiError(format!(
                    "<== ❌\n\tError api: {sub_url}, status: {status}, error: {error_msg}"
                ))),
                Err(err) => Err(Error::ParsingError(format!(
                    "<== ❌\n\tError api: {sub_url}, status: {status}, error: {}",
                    err.to_string()
                ))),
            },
            ureq::Error::Transport(e) => {
                return Err(Error::RequestError(format!(
                    "<== ❌\n\tError api: {sub_url}, error: {:?}",
                    e.to_string()
                )));
            }
        },
